<template>
  ...
  <div class="flex items-center justify-center rounded-lg p-6 shadow-md">
    ...
    <form @submit.prevent="saveUserData" class="w-[70%] space-y-6">
      ...
      <!-- Platforms -->
      <div>
        <label class="block font-medium">Platforms</label>
          ...
          <div class="flex items-center">
            ...
            >
              <div class="relative inline-block">
                <div
                  class="inputFieldComponent mt-1 flex flex-row items-center justify-center p-2 focus:outline-none"
                >
                  <input
                    v-model="user.platforms[platform][index]"
                    type="text"
                    class="mr-2 rounded-md bg-secondary"
                  />
                  <MinusCircleIcon
                    @click="removePlatformUsername(platform, index)"
                    class="h-5 w-5"
                  ></MinusCircleIcon>
                </div>
              </div>
            ...
            <PlusCircleIcon class="mt-1 h-5 w-5" @click="addPlatformUsername(platform)">
            </PlusCircleIcon>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div v-for="(url, social) in user.socials" :key="social" class="space-y-2">
          <div class="flex items-center">
            <span class="mr-4 font-medium">{{ social }}</span>
            <input
              v-model="user.socials[social]"
              type="text"
...
<script setup lang="ts">
  ...
  const addPlatformUsername = (platform: string) => {
    ...
    user.value.platforms[platform].push('') // Add a new empty username
  }

  // Remove a username from a platform
